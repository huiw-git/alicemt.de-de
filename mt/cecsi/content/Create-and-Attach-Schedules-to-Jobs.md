...
> [!NOTE]  
> Es kann nur jeweils eine einzige Instanz eines Auftrags ausgeführt werden. Wenn Sie versuchen, einen Auftrag manuell auszuführen, der bereits im Rahmen eines Zeitplans ausgeführt wird, lehnt der [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)]-Agent die Anforderung ab.  
...
-   Den [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)]-Agent-Dienst beenden.  
...
Sie müssen explizit Re\-Aktivieren Sie einen Zeitplan, der deaktiviert wurde. Bearbeiten eines Zeitplans wird nicht automatisch erneut\-Aktivieren Sie den Plan.  
  
## Planen von Startdaten  
...
## CPU-Leerlauf-Zeitpläne  
Zur maximalen Nutzung der CPU-Ressourcen können Sie eine CPU-Leerlaufbedingung für den [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)]-Agent definieren. [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)] Agent verwendet die CPU-leerlaufbedingung, um den optimalen Zeitpunkt zum Ausführen von Aufträgen festzustellen. So können Sie beispielsweise einen Zeitplan für einen Auftrag zur Neuerstellung von Indizes erstellen, der während der CPU-Leerlaufzeit und zu Zeiten mit geringer Produktion eintritt.  
  
Bevor Sie Aufträge definieren, die während der CPU-Leerlaufzeit ausgeführt werden sollen, müssen Sie die CPU-Auslastung während der normalen Verarbeitung ermitteln. Dazu können Sie mit [!INCLUDE[ssSqlProfiler](../content/includes/ssSqlProfiler_md.md)] oder mit dem Systemmonitor den Serververkehr überwachen und Statistiken erfassen. Anschließend können Sie die zusammengestellten Informationen zum Festlegen des Prozentwertes und der Dauer der CPU-Leerlaufzeit verwenden.  
  
Definieren Sie die CPU-Leerlaufbedingung als Prozentwert, unter den die CPU-Nutzung für eine bestimmte Dauer absinken muss. Legen Sie dann die Zeitdauer fest. Wenn die CPU-Nutzung für die angegebene Zeitdauer unter den angegebenen Prozentwert abfällt, startet der [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)]-Agent alle Aufträge mit CPU-Leerlaufzeitplänen. Weitere Informationen zur Verwendung von [!INCLUDE[ssSqlProfiler](../content/includes/ssSqlProfiler_md.md)] oder des Systemmonitors zur Überwachung der CPU-Nutzung finden Sie unter [Überwachen der CPU-Auslastung](assetId:///2a02a3b6-07b2-4ad0-8a24-670414d19812).  
...
|-|-|  
|**Beschreibung**|**Thema**|  
|Beschreibt, wie ein Zeitplan für einen [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)]-Agent-Auftrag erstellt wird.|[Erstellen eines Zeitplans](../content/Create-a-Schedule.md)|  
|Beschreibt, wie ein [!INCLUDE[ssNoVersion](../content/includes/ssNoVersion_md.md)]-Agent-Auftrag geplant wird.|[Planen eines Auftrags](../content/Schedule-a-Job.md)|  
|Erläutert, wie die CPU-Leerlaufbedingung für den Server definiert wird.|[Legen Sie die CPU-Leerlaufzeit und Dauer & #40; SQL Server Management Studio & #41;](../content/Set-CPU-Idle-Time-and-Duration--SQL-Server-Management-Studio-.md)|  
